...
from loaders import adult_loader, compas_loader, multi_mnist_loader, celeba_loader, credit_loader, mslr_loader, dummy_loader
from models import FullyConnected, MultiLeNet, EfficientNet, ResNet
from models import FullyConnectedCondition, MultiLeNetCondition
from models import FullyConnectedExplicit
from models import Dummy



def dataset_from_name(dataset, explicit, **kwargs):
    if explicit:
        return dummy_loader.DUMMY(**kwargs)
    ...
    if dataset == 'adult':
        ...
        return mslr_loader.MSLR(**kwargs)
    else:
        raise ValueError("Unknown dataset: {}".format(dataset))


def model_from_dataset(dataset, condition = False, explicit = False, **kwargs):
    if explicit:
        if kwargs['method'] == 'cosmos' or kwargs['method'] == 'argmo':
            return FullyConnectedExplicit(**kwargs)
        else:
            return Dummy(**kwargs)
    
    if dataset == 'adult' or dataset == 'credit' or dataset == 'compass' or dataset == 'mslr':
        return FullyConnectedCondition(**kwargs) if condition else FullyConnected(**kwargs)
    elif dataset == 'multi_mnist' or dataset == 'multi_fashion_mnist' or dataset == 'multi_fashion':
        return MultiLeNetCondition(**kwargs) if condition else MultiLeNet(**kwargs)
    elif dataset == 'celeba':
...
def dict_to_cuda(d, rank=None):
    if rank is None:
        return {k: v.cuda() if isinstance(v, torch.Tensor) else v for k, v in d.items()}
    else:
        return {k: v.to(rank) if isinstance(v, torch.Tensor) else v for k, v in d.items()}
    ...
    for i, objective in enumerate(objectives):
        ...
        for name, param in model.named_parameters():
            ...
            if not_private and param.requires_grad:
                gradients[i][name] = param.grad.detach().clone()
    
    return gradients, obj_values


def kernel_functional_rbf(losses, h=2e-2):
    pairwise_distance = torch.cdist(losses, losses)
